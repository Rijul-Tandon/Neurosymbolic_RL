...
# gymnasium: a library for creating and interacting with reinforcement learning environments (like games)
...
class Args:
    ...
    # The discount factor for future rewards. A value closer to 1 means the agent cares more about future rewards.
    gamma: float = 0.99
    """the discount factor gamma"""
    # The lambda for Generalized Advantage Estimation (GAE), a technique for estimating how much better an action is than the average action
    ...
    # The clipping coefficient for the PPO policy loss, which prevents the policy from changing too much at once
    clip_coef: float = 0.2
    """the surrogate clipping coefficient"""
    # Whether to use a clipped loss for the value function, similar to the policy loss
    clip_vloss: bool = True
    """Toggles whether or not to use a clipped loss for the value function, as per the paper."""
    # The coefficient for the entropy part of the loss, which encourages the agent to explore
    ent_coef: float = 0.01
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        # The "critic" network estimates the value of a state (how good it is to be in that state)
        ...
        # The "actor" network decides which action to take in a given state
